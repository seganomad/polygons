...
var PEEP_SIZE = 30;
var GRID_SIZE = 20;
var BASE_LIFESPAN = 50;
var LIFESPAN_BONUS = 30;
var YEAR_PER_STEP = 0.1;
		...
		if(!self.dragged){
			...
			if(neighbors>0){
				self.sameness = (same/neighbors);
                                ...
                                if(Math.random() > self.sameness)
                                {
                                    self.childColor = "pentagon"
                                }
			}else{
				self.sameness = 1;
			}
...
window.writeStats = function(){
	...
	if(segregation<0) segregation=0;
        ...
        global_segregation = segregation;
...
function step()
{
    ...
    // Get all shakers and dtf smilers
    var shaking = [];
    var smiling = [];
    ...
    // If we had a death, replace it with a birth
    if(births > 0 && empties.length > 0 && smiling.length > 0)
    {
        var parent = smiling[Math.floor(Math.random()*smiling.length)];
        
        empties.sort(function(a,b)
        {
            return distanceSquared(b,parent) - distanceSquared(a,parent);
        });
        
        var spot = empties.pop();
        
        var draggable = new Draggable(parent.x, parent.y);
        draggable.age = 0;
        draggable.lifespan = BASE_LIFESPAN + (Math.random() * LIFESPAN_BONUS);
        //draggable.color = (Math.random()<window.RATIO_TRIANGLES) ? "triangle" : "square";
        draggable.color = parent.childColor;
        draggable.childColor = draggable.color;
        draggable.gotoX = spot.x;
        draggable.gotoY = spot.y;
    ...
    // Move a random shaker
    if(shaking.length>0 && empties.length>0)
    {
